...
2. Første gang en side indlæses, hentes `config.php`, `web.config`, `cacert-2025-08-12.pem` og `.gitignore` automatisk fra Tech Colleges delte konfigurationsmappe. Programmet prøver automatisk stier baseret på din nuværende mappe (fx `https://stha2.web.techcollege.dk/Matematik/Config/`) og falder tilbage til `http://stha2.web.techcollege.dk/Config/` (med en ekstra fallback til den gamle `.../config/`-sti). Du kan supplere eller overstyre adresserne i `config/central_source.txt`.
